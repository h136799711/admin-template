<template>
    <div class="dropdown topbar-left topbar-info-item topbar-info-dropdown">
        <a href="javascript:void(0);" class="topbar-btn topbar-info-dropdown-toggle" @click.stop="dropMenu()">
            <span>{{ name }}</span>
        </a>
        <ul v-if="links.length > 0" class="topbar-info-dropdown-memu topbar-info-dropdown-memu-list" :class="{show: isDropMenu}" ref="dropMenu">
            <li v-for="(link, index) in links" class="topbar-info-btn" :key="index">
                <a :href="link.url ? link.url : 'javascript:void(0);'" @click.prevent="userMenuClick(link)">
                    <span>{{ link.name }}</span>
                </a>
            </li>
        </ul>
