<template>
    ...
    <div>
            ...
            <div class="product-nav-stage product-nav-stage-main">
                <div class="product-nav-scene product-nav-main-scene">
                    ...
                    <div class="product-nav-list">
                        <ul>
                            <li v-for="(subMenu, index) in menu.children" :key="index" :class="{ active: !subMenu.children && index === activeNavIndex }">
                                <div>
                                    <a v-if="!subMenu.children" :href="subMenu.IsFront ? routerUrl(subMenu.UrlAddress) : menuUrl(subMenu.UrlAddress) "
                                       @click.prevent="routerJump(subMenu.UrlAddress, index)">
                                        <div class="nav-icon"></div>
                                        <div class="nav-title">{{ subMenu.Name }}  </div>
                                    </a>
                                ...
                                <ul v-if="!!subMenu.children">
                                    <li v-for="(item, v) in subMenu.children" :key="`${index}-${v}`">
                                        <div>
        ...
        <div class="main-product-navbar-collapse">
            <div class="product-navbar-collapse-inner" @click="toggleSecondMenu">
                <div class="product-navbar-collapse-bg"></div>
                <div class="product-navbar-collapse">
                    <span v-if="isShowSecondMenu" class="by-icon by-icon-copy1"></span>
                    <span v-else class="by-icon by-icon-copy-copy"></span>
                </div>
            </div>
        </div>
    </div>
